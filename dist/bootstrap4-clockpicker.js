...
(function ($) {
	...
	function resolveAdaptiveVerticalPlacement(
		...
		var popoverHeight = popover.outerHeight(),
			...
			minVisibleY =
				elementTopOffset - element[0].getBoundingClientRect().top,
			...
			isEnoughRoomBelow =
				elementBottomOffset + popoverHeight <= maxVisibleY;
	...
	ClockPicker.prototype.locate = function () {
		...
		if (placement === "top-adaptive" || placement === "bottom-adaptive") {
			var preferredPlacement = placement.substr(
				0,
				placement.indexOf("-")
			);
			...
			this.updatePlacementClass(
				placement !== "viewport-top" ? placement : ""
			);
		}
		...
		switch (placement) {
			...
			case "viewport-top":
				styles.top =
					offset.top - element[0].getBoundingClientRect().top;
				break;
		}
	...
	ClockPicker.prototype.getTime = function (callback) {
		...
		return (
			(callback && callback.apply(this.element, selectedTime)) ||
			selectedTime
		);
	};
